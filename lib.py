...
def run():
    ...
    # create table book issued
        ...
        def addbook(self):
            ...
            checked = StringVar()
            drop = OptionMenu(root, checked,'fiction','horror','non-fiction','comedy','biography','thriller','autobiography','academics','Other')
                    ...
                    for i in an:
                        if i[1]>0:
                            try:
                                ...
                                conn.commit()
                                conn.close()
                                messagebox.showinfo("Updated","Book Issued sucessfully. Please return it in 7 days. Thank you!")
                            except:
    ...
    def Database():
        ...
        if cursor.fetchone() is None:
            cursor.execute("INSERT INTO `login` (username, password) VALUES('yash', 'MRIS')")
            conn.commit()
...
names = ['','ashvik']  #names, leave first empty bcz counter starts from 0
...
while True:
    ...
    for(x,y,w,h) in faces:
        ...
        if (accuracy < 100 and accuracy > 30):
            id = names[id]
